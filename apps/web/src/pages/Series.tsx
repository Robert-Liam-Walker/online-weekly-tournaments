...
export default function SeriesPage() {
  ...
  const seriesStartRef = useRef<number>(Date.now());
  ...
  // Real-time series updates via socket
  useEffect(() => {
  ...
  const winner = isCompleted
    ? series.winnerId === series.player1Id ? series.player1 : series.player2
    : null;
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="bg-gray-800 rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span
            className={`text-xs px-3 py-1 rounded-full font-medium ${
              isCompleted ? "bg-green-900 text-green-300" : "bg-yellow-900 text-yellow-300"
            }`}
          >
        ...
        {isCompleted && winner && (
          <div className="text-center py-3 bg-green-900/30 rounded-lg border border-green-800">
            <p className="text-green-300 font-semibold">{winner.username} wins the series!</p>
          </div>
        )}
        ...
        <div className="bg-gray-800 rounded-xl p-6 mb-6">
          ...
          <div className="space-y-2">
            {series.games.map((game) => {
              const gameWinner = game.winnerId === series.player1Id ? series.player1 : series.player2;
              return (
                <div
                  key={game.id}
      ...
      {/* Result submission — varies by browser */}
      {!isCompleted && isParticipant && (
        <div className="bg-gray-800 rounded-xl p-6">
          {supportsFileSystemAccess ? (
            <>
              ...
              <details className="mt-1">
                ...
                <div className="flex gap-3 items-center mt-3 flex-wrap">
                  ...
                  />
                  <button
                    onClick={() => fileRef.current?.files?.[0] && submitReplayFile(fileRef.current.files[0])}
            ...
            <>
              <h2 className="text-white font-semibold mb-1">Report Game Result</h2>
              <p className="text-gray-400 text-sm mb-4">
                After each game, report who won. Use Chrome or Edge for automatic replay detection.
              </p>
              ...
              {reportError && (
                <p className="text-red-400 text-sm mb-4 bg-red-900/30 rounded px-3 py-2">{reportError}</p>
              )}

              {/* Manual upload as alternative */}
              <details>
                <summary className="text-gray-500 text-sm cursor-pointer hover:text-gray-300">
                  Or verify with a replay file
                </summary>
                <div className="flex gap-3 items-center mt-3 flex-wrap">
                  <input
                    ref={fileRef}
                ...
                {uploadStatus === "verified" && (
                  <p className="text-green-400 text-sm mt-2">Game recorded from replay.</p>
                )}
                {uploadStatus === "error" && (
